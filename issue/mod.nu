...
alias issues-dir = match ($env | compact --empty) {
  {RAMDA_DOC_ISSUES_DIR: $d} => { $d | path expand --strict }
  _ => { root-dir | path join docs issues }
}
...
export def list [
  ...
  $ls | if ($in | is-empty) {
    ...
    insert source {|row| $row.name | path relative-to (root-dir) }
...
def build-path [slug: string]: nothing -> path {
  {parent: (issues-dir) stem: $slug extension: issue.toml} | path join
}
...
def _issue-slugs []: nothing -> list {
  let exp: glob = issues-dir | path join *.issue.toml
